...
const getVersion = (purpose: number, coinType: number) => {
    if (purpose === ScriptType.p2tr) {
        throw new Error("p2tr is not supported");
    }

    const networkType = coinType === 0 ? NetworkType.mainnet : NetworkType.testnet;
    const scriptType = purpose === ScriptType.p2pkh ? ScriptType.p2pkh :
        purpose === ScriptType.p2sh ? ScriptType.p2sh :
            purpose === ScriptType.p2wpkh ? ScriptType.p2wpkh : null;
...
class Bitcoin extends Base {
    ...
    async getExtendedPublicKey(path: string, display: boolean = true): Promise<string> {
        // decode path
            ...
            if (input.partialSig && input.partialSig.length > 0) {
                ...
                if (input.tapBip32Derivation && input.tapBip32Derivation.length > 0) {
                    ...
                    if (tapLeafHash.length > 0) {
                        return [Number(inputIndex), new PartialSignature(Buffer.from(pubkey), Buffer.from(signature), Buffer.from(tapLeafHash[0]))];
                    }
                }
    ...
    async signPsbtRaw(psbt: string | Buffer): Promise<string> {
        ...
        const psbtUR = new CryptoPSBT(psbt).toUR();
        ...
        let resultUR = parseResponoseUR(response.payload) as unknown as CryptoPSBT;
        let signedPsbt = resultUR.getPSBT();
    ...
    async signMessage(message: string, path: string): Promise<string> {
        ...
        const response = await this.sendToDevice(Actions.CMD_RESOLVE_UR, encodedUR);
        return parseSignatureUR(response.payload)
    }
...
function constructURRequest(messageBuffer: Buffer, path: string, mfp: string, type: DataType): string {
    ...
    let xfps = [mfp]
    let paths = [path]
    let cosmosUR = BtcSignRequest.constructBtcRequest(
        requestId,
        xfps,
        messageBuffer,
        type,
...
const parseSignatureUR = (ur: UR) => {
    let signature = BtcSignature.fromCBOR(ur.cbor)
