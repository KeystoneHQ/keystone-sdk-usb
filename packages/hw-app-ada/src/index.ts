...
import { CryptoKeypath, CryptoMultiAccounts, Curve, DerivationAlgorithm, KeyDerivation, KeyDerivationSchema, PathComponent, QRHardwareCall, QRHardwareCallType, QRHardwareCallVersion } from '@keystonehq/bc-ur-registry';
...
import { Network,DerivedAddress, BIP32Path, ExtendedPublicKey, HARDENED, AddressType, NativeScript, NativeScriptHashDisplayFormat, NativeScriptHash, NativeScriptType, SignTransactionRequest, SignedTransactionData, TransactionSigningMode, TxOutputDestinationType,  CertificateType, DatumType, Witness, PoolKeyType, PoolRewardAccountType, PoolOwnerType, RelayType, MessageData, SignedMessageData, MessageAddressFieldType, CIP36Vote, SignedCIP36VoteData, OperationalCertificate, OperationalCertificateSignature } from './types/public';
...
import { CredentialType, DRepType, FixLenHexString, KEY_HASH_LENGTH, ParsedAnchor, ParsedCertificate, ParsedCredential, ParsedDRep, ParsedOutputDestination, ParsedPoolParams, ParsedTransaction, SCRIPT_HASH_LENGTH, SpendingDataSourcePath, SpendingDataSourceScriptHash, SpendingDataSourceType, StakingDataSourcePath, StakingDataSourceScriptHash, StakingDataSourceType, Uint64_str, ValidBIP32Path } from './types/internal';
...
import { CardanoCertKeyData, CardanoSignature, CardanoSignRequest, CardanoUtxoData,CardanoSignCip8DataRequest,MessageAddressFieldType as KeystoneMessageAddressFieldType, CardanoSignCip8DataSignature, CardanoSignDataRequest, CardanoSignDataSignature } from '@keystonehq/bc-ur-registry-cardano';
...
import {
    Transaction as CSLTransaction, TransactionBody as CSLTransactionBody, TransactionInputs as CSLTransactionInputs, TransactionInput as CSLTransactionInput,
...
export default class Ada {
    ...
    async initAda():Promise<Map<string, ExtendedPublicKey>> {
        ...
        const path1 = parseBIP32Path([HARDENED + 1852, HARDENED + 1815, HARDENED], InvalidDataReason.INVALID_PATH);
        const path2 = parseBIP32Path([HARDENED + 1853, HARDENED + 1815, HARDENED,HARDENED], InvalidDataReason.INVALID_PATH);
    ...
    private getPublicKeyHex(path: ValidBIP32Path): string {
        ...
        if(path[0] === HARDENED + 1853) {
            const publicKeyHex = this.pathToPublicKeyMap.get(validBIP32PathToKeypathString(path))?.publicKeyHex;
            if(!publicKeyHex) {
                throw new Error('path not found in the memory cache , please call initAda function first');
            }
            return publicKeyHex;
        }
        // 1852 and other path
        ...
        const findResultKey = Array.from(keysCache).find((key) => expectedPath.includes(key));
        if( !findResultKey) {
            throw new Error('path not found in the memory cache , please call initAda function first');
        }
        ...
        const bip32PublicKeyL3 = CSLBip32PublicKey.from_bytes(
            Buffer.from(extendedPublicKey.publicKeyHex + extendedPublicKey.chainCodeHex, 'hex')
        );
    ...
    private precheck() {
        ...
        if (this.pathToPublicKeyMap.size === 0) {
            throw new Error('missing path to public key map, please call initAda first');
        }
    }
 ...
 */
        ...
        for (const path of paths) {
            ...
            if (classifyPath(path) === PathTypes.PATH_POOL_COLD_KEY) {
                ...
                const kds = new KeyDerivationSchema(
                  validBIP32PathToKeypath(parseBIP32Path(path,InvalidDataReason.INVALID_PATH)),
                  curve,
                  algo,
                  'ADA_CIP_1853',
                );
            ...
            if(classifyPath(path) === PathTypes.PATH_WALLET_SPENDING_KEY_SHELLEY || classifyPath(path) === PathTypes.PATH_WALLET_ACCOUNT){
                const kds = new KeyDerivationSchema(
                    validBIP32PathToKeypath(parseBIP32Path(path,InvalidDataReason.INVALID_PATH)),
                    curve,
                    algo,
    ...
    private async getHardwareCall(spendingPath?: ValidBIP32Path, stakingPath?: ValidBIP32Path): Promise<CryptoMultiAccounts> {
        if (!spendingPath && !stakingPath) throw new Error('spendingPath or stakingPath is required');
        const curve = Curve.ed25519;
        const algo = DerivationAlgorithm.bip32ed25519;
        const keyDerivationSchemas: KeyDerivationSchema[] = [];
        if (spendingPath) {
            keyDerivationSchemas.push(new KeyDerivationSchema(validBIP32PathToKeypath(spendingPath), curve, algo, 'ADA'));
        }
        if (stakingPath) {
            keyDerivationSchemas.push(new KeyDerivationSchema(validBIP32PathToKeypath(stakingPath), curve, algo, 'ADA'));
        }
        const hardwareCall = new QRHardwareCall(QRHardwareCallType.KeyDerivation, new KeyDerivation(keyDerivationSchemas), 'Keystone USB SDK', QRHardwareCallVersion.V1);
        const ur = hardwareCall.toUR();
        const encodedUR = new UREncoder(ur, Infinity).nextPart().toUpperCase();
        const response = await this.sendToDevice(Actions.CMD_RESOLVE_UR, encodedUR);
        const resultUR = parseResponoseUR(response.payload);
    ...
    async deriveAddress({
        ...
        let spendingScriptHash: FixLenHexString<typeof SCRIPT_HASH_LENGTH> | undefined;
        switch (address.type) {
            ...
            case AddressType.ENTERPRISE_SCRIPT:
                ...
                spendingScriptHash = (spending as SpendingDataSourceScriptHash).scriptHashHex;
                break;
        ...
        let stakingScriptHash: FixLenHexString<typeof SCRIPT_HASH_LENGTH> | undefined;
        switch (address.type) {
            case AddressType.BASE_PAYMENT_KEY_STAKE_KEY:
            case AddressType.BASE_PAYMENT_SCRIPT_STAKE_KEY:
            case AddressType.REWARD_KEY:
            ...
            case AddressType.REWARD_SCRIPT:
                ...
                stakingScriptHash = (staking as StakingDataSourceScriptHash).scriptHashHex;
                break;
            case AddressType.POINTER_KEY:
            case AddressType.POINTER_SCRIPT:
                validate(
        ...
        // eslint-disable-next-line max-len
        if (address.type === AddressType.BASE_PAYMENT_KEY_STAKE_KEY || address.type === AddressType.BASE_PAYMENT_KEY_STAKE_SCRIPT || address.type === AddressType.BASE_PAYMENT_SCRIPT_STAKE_KEY || address.type === AddressType.BASE_PAYMENT_SCRIPT_STAKE_SCRIPT) {
            ...
            if (spendingScriptHash && stakingScriptHash) {
                ...
                const header = getAddressHeader(address.type, network);
                buf = Buffer.concat([buf, header]);
                buf = Buffer.concat([buf, Buffer.from(spendingScriptHash, 'hex'), Buffer.from(stakingScriptHash, 'hex')]);
                return {
                    addressHex: buf.toString('hex'),
                };
            }
        ...
        if (address.type === AddressType.ENTERPRISE_KEY || address.type === AddressType.ENTERPRISE_SCRIPT) {
            if (spendingPath) {
                const accounts = await this.getHardwareCall(spendingPath);
                const blake2b = blake2.createHash('blake2b', { digestLength: 28 });
        ...
        if (address.type === AddressType.REWARD_KEY || address.type === AddressType.REWARD_SCRIPT) {
            if (stakingPath) {
                const accounts = await this.getHardwareCall(stakingPath);
                const blake2b = blake2.createHash('blake2b', { digestLength: 28 });
                blake2b.update(accounts.getKeys()[0].getKey());
   ...
   */
    ...
    async signTransaction(
        ...
        const  extraCertSigners = this.prepareExtraSiners(parsedRequest.tx.certificates,mfp);
        if(parsedRequest.additionalWitnessPaths.length > 0){
            const keyPathList = parsedRequest.additionalWitnessPaths.map((path)=>validBIP32PathToKeypathString(path));
        ...
        const rightTx = cardanoSerialization.Transaction.from_hex('');
        ...
        const witnessSet = cardanoSerialization.TransactionWitnessSet.from_bytes(sig.getWitnessSet());
        const vkeywitnesses:cardanoSerialization.Vkeywitnesses | undefined = witnessSet.vkeys();    
    ...
    async signMessage(request: SignMessageRequest): Promise<SignMessageResponse> {
        ...
        const xfp = this.mfp;
        if(!xfp){
            throw new Error('mfp not found , keystone 3 pro need xfp to identify the device');
        }
        const xpub =  this.getPublicKeyHex(parsedMsgData.signingPath);
        const hashPayload = parsedMsgData.hashPayload;
        const addressFieldType = parsedMsgData.addressFieldType === MessageAddressFieldType.ADDRESS ? KeystoneMessageAddressFieldType.ADDRESS : KeystoneMessageAddressFieldType.KEY_HASH;
        // address bench32 
        const addressHex = request.addressFieldType === MessageAddressFieldType.ADDRESS ? (await this.deriveAddress({network:request.network,address:request.address})).addressHex : null;
        const addressBench32 = addressHex ? bech32_encodeAddress(Buffer.from(addressHex,'hex')) : '';
        // todo : now , we need sleep 10s to close the page on the device
        await new Promise(resolve => setTimeout(resolve, 10000));
        const ur = CardanoSignCip8DataRequest.constructCardanoSignCip8DataRequest(signData, hePath, xfp, xpub, hashPayload, addressFieldType, addressBench32, uuidString, 'Keystone USB SDK').toUR(); 
        const encodedUR = new UREncoder(ur, Infinity).nextPart().toUpperCase();
        const response = await this.sendToDevice(Actions.CMD_RESOLVE_UR, encodedUR);
        const resultUR = parseResponoseUR(response.payload);
    ...
    async signCIP36Vote(
        ...
        const ur = CardanoSignDataRequest.constructCardanoSignDataRequest(signData, hePath, xfp, xpub, uuidString, 'Keystone USB SDK').toUR();
        const encodedUR = new UREncoder(ur, Infinity).nextPart().toUpperCase();
        const response = await this.sendToDevice(Actions.CMD_RESOLVE_UR, encodedUR);
        const resultUR = parseResponoseUR(response.payload);
   ...
   */
    ...
    async signOperationalCertificate(
      ...
      const hdPath = validBIP32PathToKeypathString(parsedOperationalCertificate.coldKeyPath);
      const xfp = this.mfp;
      if(!xfp){
        throw new Error('mfp not found , keystone 3 pro need xfp to identify the device');
      }
      ...
      const ur = await CardanoSignDataRequest.constructCardanoSignDataRequest(signData, hdPath, xfp, xpub, uuidString, origin).toUR();
      const encodedUR = new UREncoder(ur, Infinity).nextPart().toUpperCase();
      const response = await this.sendToDevice(Actions.CMD_RESOLVE_UR, encodedUR);
      const resultUR = parseResponoseUR(response.payload);
      const result = CardanoSignDataSignature.fromCBOR(resultUR.cbor);
    ...
    private convertToCSLTransaction(tx: ParsedTransaction): CSLTransaction {
        ...
        tx.inputs.forEach((input) => {
            inputs.add(CSLTransactionInput.new(CSLTransactionHash.from_hex(input.txHashHex), input.outputIndex));
        });
        ...
        tx.outputs.forEach((output) => {
            switch (output.destination.type) {
                case TxOutputDestinationType.THIRD_PARTY:
                    {
                        const cslOutput = CSLTransactionOutput.new(CSLAddress.from_hex(output.destination.addressHex), CSLValue.new(CSLBigNum.from_str(output.amount.toString())));
                        if (output.datum) {
                            if(output.datum.type === DatumType.HASH){
                                cslOutput.set_data_hash(CSLDataHash.from_hex(output.datum.datumHashHex));
                            } else if(output.datum.type === DatumType.INLINE){
                                cslOutput.set_plutus_data(CSLPlutusData.from_hex(output.datum.datumHex));
                            }
                        }
                        if (output.referenceScriptHex) {
                            ...
                            let bytesData;
                            if (dataLength <= 23) {
                                bytesData = (0x40 + dataLength).toString(16) + output.referenceScriptHex;
                            } else if (dataLength <= 255) {
                                bytesData = '58' + dataLength.toString(16).padStart(2, '0') + output.referenceScriptHex;
                            } else if (dataLength <= 65535) {
                                bytesData = '59' + dataLength.toString(16).padStart(4, '0') + output.referenceScriptHex;
                            } else {
                                bytesData = '5a' + dataLength.toString(16).padStart(8, '0') + output.referenceScriptHex;
                            }
                            const finalScriptRefBuf = Buffer.concat([Buffer.from(tagPrefix,'hex'),Buffer.from(bytesData,'hex')]);
                            cslOutput.set_script_ref(cardanoSerialization.ScriptRef.from_bytes(finalScriptRefBuf));
                        }
                        outputs.add(cslOutput);
                    }
                    break;
                case TxOutputDestinationType.DEVICE_OWNED:
                    throw new Error('Keystone 3 Pro does not support device owned address');
                default:
        ...
        tx.certificates.forEach((certificate) => {
            switch(certificate.type){
                case CertificateType.STAKE_DELEGATION:
                    certificates.add(CSLCertificate.new_stake_delegation(this.prepareStakeDelegation(certificate)));
                    break;
                case CertificateType.STAKE_DEREGISTRATION_CONWAY:
                    certificates.add(CSLCertificate.new_stake_registration_and_delegation(this.prepareStakeDeregistrationConway(certificate)));
                    break;
                case CertificateType.STAKE_DEREGISTRATION:
                    certificates.add(CSLCertificate.new_stake_deregistration(this.prepareStakeDeregistration(certificate)));
                    break;
                case CertificateType.STAKE_REGISTRATION:
                    certificates.add(CSLCertificate.new_stake_registration(this.prepareStakeRegistration(certificate)));
                    break;
                case CertificateType.STAKE_REGISTRATION_CONWAY:
                    certificates.add(CSLCertificate.new_stake_registration_and_delegation(this.prepareStakeRegistrationConway(certificate)));
                    break;
                case CertificateType.VOTE_DELEGATION:
                    certificates.add(CSLCertificate.new_vote_delegation(this.prepareVoteDelegation(certificate)));
                    break;
                case CertificateType.AUTHORIZE_COMMITTEE_HOT:
                    certificates.add(CSLCertificate.new_committee_hot_auth(this.prepareAuthorizeCommitteeHot(certificate)));
                    break;
                case CertificateType.RESIGN_COMMITTEE_COLD:
                    certificates.add(CSLCertificate.new_committee_cold_resign(this.prepareResignCommitteeCold(certificate)));
                    break;
                case CertificateType.DREP_REGISTRATION:
                    certificates.add(CSLCertificate.new_drep_registration(this.prepareDrepRegistration(certificate)));
                    break;
                case CertificateType.DREP_DEREGISTRATION:
                    certificates.add(CSLCertificate.new_drep_deregistration(this.prepareDrepDeregistration(certificate)));
                    break;
                case CertificateType.DREP_UPDATE:
                    certificates.add(CSLCertificate.new_drep_update(this.prepareDrepUpdate(certificate)));
                    break;
                case CertificateType.STAKE_POOL_REGISTRATION:
                    certificates.add(CSLCertificate.new_pool_registration(this.prepareStakePoolRegistration(certificate)));
                    break;
                case CertificateType.STAKE_POOL_RETIREMENT:
                    certificates.add(CSLCertificate.new_pool_retirement(this.prepareStakePoolRetirement(certificate)));
                    break;
            }
        });
        ...
        const witness_set: CSLTransactionWitnessSet = CSLTransactionWitnessSet.new();
        const cslTx: CSLTransaction = CSLTransaction.new(
            body,
            witness_set,
        );
        return cslTx;
    ...
    private  prepareStakePoolRetirement(certificate: { type: CertificateType.STAKE_POOL_RETIREMENT; path: ValidBIP32Path; retirementEpoch: Uint64_str; }): cardanoSerialization.PoolRetirement {
        ...
        const poolKeyHash = cardanoSerialization.Ed25519KeyHash.from_hex(addressHex);
        const poolRetirement : cardanoSerialization.PoolRetirement = cardanoSerialization.PoolRetirement.new(poolKeyHash, Number(certificate.retirementEpoch));
        return poolRetirement;
    }
 

    private prepareStakePoolRegistration(certificate: { type: CertificateType.STAKE_POOL_REGISTRATION; pool: ParsedPoolParams; }): cardanoSerialization.PoolRegistration {
        ...
        case PoolRewardAccountType.THIRD_PARTY:
            rewardAccount = cardanoSerialization.RewardAddress.from_address(cardanoSerialization.Address.from_hex(certificate.pool.rewardAccount.rewardAccountHex));
            break;
        case PoolRewardAccountType.DEVICE_OWNED:
            rewardAccount = cardanoSerialization.RewardAddress.from_address(cardanoSerialization.Address.from_hex(this.getPublicKeyHex(certificate.pool.rewardAccount.path)));
            break;
    }
    if(!rewardAccount){
        throw new Error('Reward account is not set');
    }
    const owners : cardanoSerialization.Ed25519KeyHashes = cardanoSerialization.Ed25519KeyHashes.new();
    ...
    const relays : cardanoSerialization.Relays = cardanoSerialization.Relays.new();
    certificate.pool.relays.forEach((relay) => {
        if(relay.type === RelayType.SINGLE_HOST_HOSTNAME){
            relays.add(cardanoSerialization.Relay.new_single_host_name(cardanoSerialization.SingleHostName.new(Number(relay.port),cardanoSerialization.DNSRecordAorAAAA.new(relay.dnsName))));
        } else if(relay.type === RelayType.SINGLE_HOST_IP_ADDR){
            if(relay.ipv4){
                relays.add(cardanoSerialization.Relay.new_single_host_addr(cardanoSerialization.SingleHostAddr.new(Number(relay.port),cardanoSerialization.Ipv4.new(Buffer.from(relay.ipv4)))));
            } 
            if(relay.ipv6){
                relays.add(cardanoSerialization.Relay.new_single_host_addr(cardanoSerialization.SingleHostAddr.new(Number(relay.port),undefined,cardanoSerialization.Ipv6.new(Buffer.from(relay.ipv6)))));
            }
            if(relay.ipv4 && relay.ipv6){
                relays.add(cardanoSerialization.Relay.new_single_host_addr(cardanoSerialization.SingleHostAddr.new(Number(relay.port),cardanoSerialization.Ipv4.new(Buffer.from(relay.ipv4)),cardanoSerialization.Ipv6.new(Buffer.from(relay.ipv6)))));
            }
        } else if(relay.type === RelayType.MULTI_HOST){
            relays.add(cardanoSerialization.Relay.new_multi_host_name(cardanoSerialization.MultiHostName.new(cardanoSerialization.DNSRecordSRV.new(relay.dnsName))));
        }
    });
    switch(certificate.pool.poolKey.type){
        case PoolKeyType.THIRD_PARTY:
            poolParams = cardanoSerialization.PoolParams.new(
                cardanoSerialization.Ed25519KeyHash.from_hex(certificate.pool.poolKey.hashHex),
                ...
                cardanoSerialization.UnitInterval.new(cardanoSerialization.BigNum.from_str(certificate.pool.margin.numerator), cardanoSerialization.BigNum.from_str(certificate.pool.margin.denominator)),
                rewardAccount,
                owners,
                relays,
            );
        ...
        case PoolKeyType.DEVICE_OWNED:
            poolParams = cardanoSerialization.PoolParams.new(
                cardanoSerialization.Ed25519KeyHash.from_hex(this.getPublicKeyHex(certificate.pool.poolKey.path)),
                cardanoSerialization.VRFKeyHash.from_hex(certificate.pool.vrfHashHex),
                cardanoSerialization.BigNum.from_str(certificate.pool.pledge),
                cardanoSerialization.BigNum.from_str(certificate.pool.cost),
                cardanoSerialization.UnitInterval.new(cardanoSerialization.BigNum.from_str(certificate.pool.margin.numerator), cardanoSerialization.BigNum.from_str(certificate.pool.margin.denominator)),
    ...
    const poolRegistration : cardanoSerialization.PoolRegistration = cardanoSerialization.PoolRegistration.new(poolParams);
    return poolRegistration;
}


    private prepareDrepUpdate(certificate: { type: CertificateType.DREP_UPDATE; dRepCredential: ParsedCredential; anchor: ParsedAnchor | null; }): cardanoSerialization.DRepUpdate {
        let dRepCredential : cardanoSerialization.Credential;
        switch(certificate.dRepCredential.type){
            case CredentialType.KEY_HASH:
            dRepCredential = cardanoSerialization.Credential.from_keyhash(cardanoSerialization.Ed25519KeyHash.from_hex(certificate.dRepCredential.keyHashHex));
            break;
        case CredentialType.KEY_PATH:
            dRepCredential = cardanoSerialization.Credential.from_keyhash(cardanoSerialization.Ed25519KeyHash.from_hex(this.getPublicKeyHex(certificate.dRepCredential.path)));
            break;
        case CredentialType.SCRIPT_HASH:
            dRepCredential = cardanoSerialization.Credential.from_scripthash(cardanoSerialization.ScriptHash.from_hex(certificate.dRepCredential.scriptHashHex));
            break;
    }
    if(certificate.anchor){
        const anchor = cardanoSerialization.Anchor.from_hex(certificate.anchor.hashHex);
        return cardanoSerialization.DRepUpdate.new_with_anchor(dRepCredential, anchor);
    }
    ...
    private prepareDrepDeregistration(certificate: { type: CertificateType.DREP_DEREGISTRATION; dRepCredential: ParsedCredential; deposit: Uint64_str; }): cardanoSerialization.DRepDeregistration {
        let dRepCredential : cardanoSerialization.Credential;
        switch(certificate.dRepCredential.type){
            case CredentialType.KEY_HASH:
            dRepCredential = cardanoSerialization.Credential.from_keyhash(cardanoSerialization.Ed25519KeyHash.from_hex(certificate.dRepCredential.keyHashHex));
    ...
    return cardanoSerialization.DRepDeregistration.new(dRepCredential, cardanoSerialization.BigNum.from_str(certificate.deposit));
}

    private prepareDrepRegistration(certificate: { type: CertificateType.DREP_REGISTRATION; dRepCredential: ParsedCredential; deposit: Uint64_str; anchor: ParsedAnchor | null; }): cardanoSerialization.DRepRegistration {
        let dRepCredential : cardanoSerialization.Credential;
        switch(certificate.dRepCredential.type){
            case CredentialType.KEY_HASH:
            dRepCredential = cardanoSerialization.Credential.from_keyhash(cardanoSerialization.Ed25519KeyHash.from_hex(certificate.dRepCredential.keyHashHex));
    ...
    if(certificate.anchor){
        const anchor = cardanoSerialization.Anchor.from_hex(certificate.anchor.hashHex);
        return cardanoSerialization.DRepRegistration.new_with_anchor(dRepCredential, cardanoSerialization.BigNum.from_str(certificate.deposit), anchor);
    }
    ...
    private prepareResignCommitteeCold(certificate: { type: CertificateType.RESIGN_COMMITTEE_COLD; coldCredential: ParsedCredential; anchor: ParsedAnchor | null; }): cardanoSerialization.CommitteeColdResign {
        let coldCredential : cardanoSerialization.Credential;
        switch(certificate.coldCredential.type){
            case CredentialType.KEY_HASH:
            coldCredential = cardanoSerialization.Credential.from_keyhash(cardanoSerialization.Ed25519KeyHash.from_hex(certificate.coldCredential.keyHashHex));
            break;
        case CredentialType.KEY_PATH:
            coldCredential = cardanoSerialization.Credential.from_keyhash(cardanoSerialization.Ed25519KeyHash.from_hex(this.getPublicKeyHex(certificate.coldCredential.path)));
            break;
        case CredentialType.SCRIPT_HASH:
            coldCredential = cardanoSerialization.Credential.from_scripthash(cardanoSerialization.ScriptHash.from_hex(certificate.coldCredential.scriptHashHex));
            break;
    }
    
    if(certificate.anchor){
        const anchor = cardanoSerialization.Anchor.from_hex(certificate.anchor.hashHex);
        return cardanoSerialization.CommitteeColdResign.new_with_anchor(coldCredential, anchor);
    }
    return cardanoSerialization.CommitteeColdResign.new(coldCredential);
}


    private prepareAuthorizeCommitteeHot(certificate: { type: CertificateType.AUTHORIZE_COMMITTEE_HOT; coldCredential: ParsedCredential; hotCredential: ParsedCredential; }): cardanoSerialization.CommitteeHotAuth {
        let coldCredential : cardanoSerialization.Credential;
        switch(certificate.coldCredential.type){
            case CredentialType.KEY_HASH:
            coldCredential = cardanoSerialization.Credential.from_keyhash(cardanoSerialization.Ed25519KeyHash.from_hex(certificate.coldCredential.keyHashHex));
    ...
    switch(certificate.hotCredential.type){
        case CredentialType.KEY_HASH:
            hotCredential = cardanoSerialization.Credential.from_keyhash(cardanoSerialization.Ed25519KeyHash.from_hex(certificate.hotCredential.keyHashHex));
            break;
        case CredentialType.KEY_PATH:
            hotCredential = cardanoSerialization.Credential.from_keyhash(cardanoSerialization.Ed25519KeyHash.from_hex(this.getPublicKeyHex(certificate.hotCredential.path)));
            break;
        case CredentialType.SCRIPT_HASH:
            hotCredential = cardanoSerialization.Credential.from_scripthash(cardanoSerialization.ScriptHash.from_hex(certificate.hotCredential.scriptHashHex));
            break;
    }
    const committeeHotAuth : cardanoSerialization.CommitteeHotAuth = cardanoSerialization.CommitteeHotAuth.new(coldCredential, hotCredential);
    return committeeHotAuth;
}


    private prepareVoteDelegation(certificate: { type: CertificateType.VOTE_DELEGATION; stakeCredential: ParsedCredential; dRep: ParsedDRep; }): cardanoSerialization.VoteDelegation {
        ...
        case DRepType.KEY_PATH:
            dRepCredential = cardanoSerialization.DRep.new_from_credential(cardanoSerialization.Credential.from_keyhash(cardanoSerialization.Ed25519KeyHash.from_hex(this.getPublicKeyHex(certificate.dRep.path))));
            break;
        case DRepType.KEY_HASH:
            dRepCredential = cardanoSerialization.DRep.new_key_hash(cardanoSerialization.Ed25519KeyHash.from_hex(certificate.dRep.keyHashHex));
            break;
        case DRepType.SCRIPT_HASH:
            dRepCredential = cardanoSerialization.DRep.new_script_hash(cardanoSerialization.Ed25519KeyHash.from_hex(certificate.dRep.scriptHashHex));
            break;
    ...
    switch(certificate.stakeCredential.type){
        case CredentialType.KEY_HASH:
            voteDelegation = cardanoSerialization.VoteDelegation.new(
                cardanoSerialization.Credential.from_keyhash(cardanoSerialization.Ed25519KeyHash.from_hex(certificate.stakeCredential.keyHashHex)),
                dRepCredential,
            );
            break;
        case CredentialType.KEY_PATH:
            voteDelegation = cardanoSerialization.VoteDelegation.new(
                cardanoSerialization.Credential.from_keyhash(cardanoSerialization.Ed25519KeyHash.from_hex(this.getPublicKeyHex(certificate.stakeCredential.path))),
                dRepCredential,
            );
            break;
        case CredentialType.SCRIPT_HASH:
            voteDelegation = cardanoSerialization.VoteDelegation.new(
                cardanoSerialization.Credential.from_hex(certificate.stakeCredential.scriptHashHex),
                dRepCredential,
            );
            break;
    }
    ...
    private prepareStakeRegistrationConway(certificate: { type: CertificateType.STAKE_REGISTRATION_CONWAY; stakeCredential: ParsedCredential; deposit: Uint64_str; }): cardanoSerialization.StakeRegistrationAndDelegation {
        let stakeRegistrationConway : cardanoSerialization.StakeRegistrationAndDelegation;
        switch(certificate.stakeCredential.type){
            case CredentialType.KEY_HASH:
            stakeRegistrationConway = cardanoSerialization.StakeRegistrationAndDelegation.new(
                cardanoSerialization.Credential.from_keyhash(cardanoSerialization.Ed25519KeyHash.from_hex(certificate.stakeCredential.keyHashHex)),
                cardanoSerialization.Ed25519KeyHash.from_hex(certificate.stakeCredential.keyHashHex),
                cardanoSerialization.BigNum.from_str(certificate.deposit)
            );
            break;
        case CredentialType.KEY_PATH:
            stakeRegistrationConway = cardanoSerialization.StakeRegistrationAndDelegation.new(
                cardanoSerialization.Credential.from_keyhash(cardanoSerialization.Ed25519KeyHash.from_hex(this.getPublicKeyHex(certificate.stakeCredential.path))),
                cardanoSerialization.Ed25519KeyHash.from_hex(this.getPublicKeyHex(certificate.stakeCredential.path)),
                cardanoSerialization.BigNum.from_str(certificate.deposit)
            );
            break;
        case CredentialType.SCRIPT_HASH:
            stakeRegistrationConway = cardanoSerialization.StakeRegistrationAndDelegation.new(
                cardanoSerialization.Credential.from_scripthash(cardanoSerialization.ScriptHash.from_hex(certificate.stakeCredential.scriptHashHex)),
                cardanoSerialization.Ed25519KeyHash.from_hex(certificate.stakeCredential.scriptHashHex),
                cardanoSerialization.BigNum.from_str(certificate.deposit)
            );
            break;
    }
    ...
    private prepareStakeRegistration(certificate: { type: CertificateType.STAKE_REGISTRATION; stakeCredential: ParsedCredential; }): cardanoSerialization.StakeRegistration {
    ...
    private prepareStakeDeregistration(certificate: { type: CertificateType.STAKE_DEREGISTRATION; stakeCredential: ParsedCredential; }): cardanoSerialization.StakeDeregistration {
    ...
    private prepareStakeDelegation(certificate: { type: CertificateType.STAKE_DELEGATION; stakeCredential: ParsedCredential; poolKeyHashHex: FixLenHexString<typeof KEY_HASH_LENGTH>; }): cardanoSerialization.StakeDelegation {
        let stakeDelegation : cardanoSerialization.StakeDelegation;
        switch(certificate.stakeCredential.type){
            case CredentialType.KEY_HASH:
            stakeDelegation = cardanoSerialization.StakeDelegation.new(
                cardanoSerialization.Credential.from_keyhash(cardanoSerialization.Ed25519KeyHash.from_hex(certificate.stakeCredential.keyHashHex)),
                cardanoSerialization.Ed25519KeyHash.from_hex(certificate.poolKeyHashHex)
            );
            break;
        case CredentialType.KEY_PATH:
            stakeDelegation = cardanoSerialization.StakeDelegation.new(
    ...
    private prepareStakeDeregistrationConway(certificate: { type: CertificateType.STAKE_DEREGISTRATION_CONWAY; stakeCredential: ParsedCredential; deposit: Uint64_str; }): cardanoSerialization.StakeRegistrationAndDelegation {
        let stakeDelegationConway : cardanoSerialization.StakeRegistrationAndDelegation;
        switch(certificate.stakeCredential.type){
            case CredentialType.KEY_HASH:
        stakeDelegationConway = cardanoSerialization.StakeRegistrationAndDelegation.new(
            cardanoSerialization.Credential.from_keyhash(cardanoSerialization.Ed25519KeyHash.from_hex(certificate.stakeCredential.keyHashHex)),
            cardanoSerialization.Ed25519KeyHash.from_hex(certificate.stakeCredential.keyHashHex),
            cardanoSerialization.BigNum.from_str(certificate.deposit)
        );
    ...
    case CredentialType.KEY_PATH:
        stakeDelegationConway = cardanoSerialization.StakeRegistrationAndDelegation.new(
            cardanoSerialization.Credential.from_keyhash(cardanoSerialization.Ed25519KeyHash.from_hex(validBIP32PathToKeypathString(certificate.stakeCredential.path))),
            cardanoSerialization.Ed25519KeyHash.from_hex(this.getPublicKeyHex(certificate.stakeCredential.path)),
            cardanoSerialization.BigNum.from_str(certificate.deposit)
        );
        break;
    ...
    private prepareExtraSiners(certs:ParsedCertificate[],mfp:string) : CardanoCertKeyData[]{
        const keyHash = '';
        const xfp = mfp;
        ...
        switch(certificate.type){
            case CertificateType.STAKE_DELEGATION:
                if(certificate.stakeCredential.type === CredentialType.KEY_PATH){
                    const keypath = validBIP32PathToKeypathString(certificate.stakeCredential.path);
            ...
            case CertificateType.AUTHORIZE_COMMITTEE_HOT:
                if(certificate.coldCredential.type === CredentialType.KEY_PATH){
                    const keypath = validBIP32PathToKeypathString(certificate.coldCredential.path);
                    result.push({keyHash,xfp,keyPath:keypath});
                }
                break;
            case CertificateType.RESIGN_COMMITTEE_COLD:
            ...
            case CertificateType.DREP_REGISTRATION:
                if(certificate.dRepCredential.type === CredentialType.KEY_PATH){
                    const keypath = validBIP32PathToKeypathString(certificate.dRepCredential.path);
                    result.push({keyHash,xfp,keyPath:keypath});
                }
                break;
            case CertificateType.DREP_DEREGISTRATION:
            ...
            case CertificateType.DREP_UPDATE:
                ...
                break;
            case CertificateType.STAKE_POOL_REGISTRATION:
                if(certificate.pool.rewardAccount.type === PoolRewardAccountType.DEVICE_OWNED){
                    const keypath = validBIP32PathToKeypathString(certificate.pool.rewardAccount.path);
                    result.push({keyHash,xfp,keyPath:keypath});
                }
                break;
            case CertificateType.STAKE_POOL_RETIREMENT:
    ...
    switch (nativeScript.type) {
        ...
        case NativeScriptType.INVALID_BEFORE: {
            const slot = cardanoSerialization.TimelockStart.new_timelockstart(
                cardanoSerialization.BigNum.from_str(nativeScript.params.slot.toString()),
            );
...
const getAddressHeader = (addressType: AddressType, network: Network): Buffer => {
...
const constructSignTransactionURRequest = (signData: Buffer, utxos: CardanoUtxoData[], extraSigners: CardanoCertKeyData[], origin: string): UR => {
